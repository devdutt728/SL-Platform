...
if "%BUILD_ON_START%"=="1" (
  ...
  if "%ENABLE_PROJECT_PLANNER%"=="1" if exist "%PLANNER_FRONTEND_DIR%" (
    powershell -NoProfile -Command "$ErrorActionPreference='Stop'; Set-Location -LiteralPath '%PLANNER_FRONTEND_DIR%'; npm run build 2>&1 | Tee-Object -FilePath '%PLANNER_FRONTEND_BUILD_OUT%'; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }"
    if errorlevel 1 goto :fail
  )
)
