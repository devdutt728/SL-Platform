  ...
  return (
    <aside className={`glass-panel flex flex-col gap-3 rounded-2xl p-4 ${className ?? ""}`}>
      ...
      <div className="space-y-2">
        {loading && items.length === 0 ? <p className="text-xs text-slate-500">Loading activity...</p> : null}
        {items.map((event) => {
          ...
          return (
            <div key={event.event_id} className="rounded-xl border border-white/50 bg-white/40 p-3 shadow-md">
              <div className="flex items-center gap-2">
                <Icon className="h-4 w-4 text-violet-500" />
                <p className="text-sm font-medium">{prettifyAction(event.action_type)}</p>
              </div>
              <p className="mt-1 text-xs text-slate-600">{candidateLabel} - {by}</p>
              <p className="text-xs text-slate-500">{formatDateTime(event.created_at)}</p>
            </div>
          );
        })}
        {!loading && items.length === 0 ? <p className="text-xs text-slate-500">No recent activity yet.</p> : null}
      </div>
    </aside>
  );
}
