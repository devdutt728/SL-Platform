...
function getStatusBadge(status?: string | null, isDeleted?: number | null) {
  if (isDeleted) {
...
export function SuperAdminPeopleClient() {
  ...
  return (
    <section className="space-y-5">
      <section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-white via-white to-amber-50/70 p-6 shadow-sm">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4">
          <div className="space-y-2">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">SuperAdmin Console</p>
          ...
          <div className="min-w-[220px] rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 text-xs text-slate-600 shadow-sm">
            ...
            <p className="mt-1 truncate text-xs text-slate-500">{formName || "No record loaded"}</p>
            {formStatusBadge ? (
              <span className={`mt-2 inline-flex rounded-full border px-2 py-0.5 text-[11px] ${formStatusBadge.className}`}>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <section className="relative overflow-visible rounded-[32px] border border-slate-200/70 bg-white/70 p-6 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.55)]">
          <div className="absolute -left-10 top-6 h-24 w-24 rounded-full bg-cyan-200/40 blur-3xl" aria-hidden="true" />
          <div className="absolute -right-16 bottom-2 h-32 w-32 rounded-full bg-emerald-200/40 blur-[64px]" aria-hidden="true" />
          <div className="relative flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-[0.35em] text-slate-500">People Search</p>
            ...
            <button
              type="button"
              onClick={resetForm}
              className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/80 px-4 py-2 text-xs font-semibold text-slate-700 shadow-sm"
            >
          ...
          <div className="relative mt-5 grid gap-4">
            <label className="space-y-1 text-xs text-slate-600">
              ...
              <div className="relative" ref={menuRef}>
                ...
                <input
                  className="w-full rounded-2xl border border-slate-200/80 bg-slate-900/5 py-3 pl-10 pr-24 text-sm text-slate-800 shadow-inner focus:border-cyan-200 focus:outline-none focus:ring-2 focus:ring-cyan-100"
                ...
                />
                <div className="pointer-events-none absolute right-3 top-3 text-[10px] font-semibold uppercase tracking-[0.3em] text-slate-400">
                  live
                </div>
                {loadingOptions ? <div className="absolute right-3 top-11 text-xs text-slate-400">Indexing...</div> : null}
                {!selected && open && options.length > 0 ? (
                  <div className="absolute z-10 mt-2 w-full max-h-72 overflow-auto rounded-2xl border border-slate-200/80 bg-white/95 shadow-[0_20px_40px_-30px_rgba(15,23,42,0.6)]">
                    {options.map((person) => {
                ...
                {!selected && open && !loadingOptions && options.length === 0 && query.trim().length > 1 ? (
                  <div className="absolute z-10 mt-2 w-full rounded-2xl border border-slate-200/80 bg-white px-4 py-3 text-xs text-slate-500 shadow-card">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white/70 p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className="rounded-full bg-slate-900/5 p-2 text-slate-600">
...
function SectionHeader({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-2">
      <div>
        <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-slate-400">{title}</p>
